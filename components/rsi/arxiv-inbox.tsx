...
type FeedItem = {
  ...
  category: string; topics?: string[]; paperType?: string; classificationReason: string; matchedTerms: string[];
};
...
export function ArxivInbox() {
  ...
  const matches = useMemo(() => {
    ...
    return items.filter(p => (category === 'All Papers' || (p.topics ?? [p.category]).includes(category) || (category === 'Unclassified' && !p.topics?.length)) && words.every(w => [p.title, p.abstract, p.id, p.category, ...(p.topics ?? []), p.paperType ?? '', ...p.authors, ...p.matchedTerms].join(' ').toLowerCase().includes(w)))
  ...
  return <section id="arxiv-feed" className="arxiv-section" aria-labelledby="arxiv-title">
    ...
    <div className="feed-controls"><label className="feed-search"><Search size={16}/><input type="search" aria-label="Search arXiv Feed" placeholder="Search titles, authors, abstracts…" value={query} onChange={e => {setQuery(e.target.value); setLimit(6);}}/></label><select aria-label="arXiv Feed category" value={category} onChange={e => {setCategory(e.target.value); setLimit(6);}}>{[...categories,'Unclassified'].map(c => <option key={c}>{c}</option>)}</select></div>
    ...
    <div className="feed-list">{matches.slice(0, limit).map(p => <article className="feed-paper" key={p.id}>
      <div className="feed-paper-meta"><span className="category-tag">{p.paperType ?? 'Unclassified'}</span>{(p.topics?.length ? p.topics : ['Unclassified']).map(t => <span className="topic-tag" key={t}>{t}</span>)}<span className="feed-provisional">Provisional</span><span className="feed-id">arXiv:{p.id} · {p.version}</span></div>
