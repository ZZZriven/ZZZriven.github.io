...
export async function generateMetadata({params}: {params: Promise<{id: string}>}): Promise<Metadata> {
  ...
  if (!p) return {title: '论文未找到 · RSI 研究库', robots: {index: false}};
  return {title: p.title + ' · RSI 研究库', description: p.summary, alternates: {canonical: '/rsi/papers/' + id + '/'}};
}
export default async function PaperPage({params}: {params: Promise<{id: string}>}) {
  ...
  return <main id="main" className="reading-main">
    ...
    <div className="reading-layout"><article className="reading-article">
      <header className="reading-header"><div className="reading-labels"><a className="category-tag" href={'/rsi/?category=' + encodeURIComponent(p.category) + '#library'}>{p.category}</a><span className={'mode-tag mode-' + p.mode}>{modeLabel(p.mode)}</span></div><h1>{p.title}</h1><p className="reading-english">{p.en}</p><p className="reading-authors">{p.authors}</p><div className="reading-dates"><span>首次提交 <time dateTime={p.date}>{formatDate(p.date)}</time></span>{p.updated && <span>已核实修订 <time dateTime={p.updated}>{formatDate(p.updated)}</time> · {p.version}</span>}<span>解析整理 <time dateTime={p.reviewedAt}>{formatDate(p.reviewedAt)}</time></span></div><div className="reading-links"><a className="rsi-button primary" href={source} target="_blank" rel="noreferrer"><BookOpen size={15}/>arXiv 原文<ArrowUpRight size={14}/></a><a className="rsi-button" href={pdf} target="_blank" rel="noreferrer"><FileText size={15}/>阅读 PDF<ArrowUpRight size={14}/></a></div></header>
      ...
      <a className="reading-return" href="/rsi/#library"><ArrowLeft size={16}/>返回研究库</a>
